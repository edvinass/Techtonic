...
describe("citizen water walk loop", () => {
  it("treats a river circumnavigation as a detour", () => {
    ...
    // Seal a full-height water wall; only one gap at the far south
  ...
  it("does not send foragers on a shore loop across a river", () => {
    ...
    // Clear a neighbourhood, then cut a river east of home with fertile beyond
    ...
    // Gap only at the far north — long detour
    state.map.tiles[1 * state.map.width + (home.x + 4)].terrain = "grass";

    // Fertile just across the river (manhattan-close, path-far)
    const far = state.map.tiles[home.y * state.map.width + (home.x + 6)];
    far.terrain = "fertile";
    far.deposit = null;
    far.stock = 40;

    // Fertile on the home side (still within forage range 2–7)
    ...
    for (const c of gatherers) {
      ...
      // Must not target the across-river bank
